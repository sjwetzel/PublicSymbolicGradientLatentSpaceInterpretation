...
myarrayX=readdlm("gradient_data/X.out", ',', Float64)
myarrayGradients=readdlm("gradient_data/gradients.out", ',', Float64)
...
model = SRRegressor(;
    ...
    unary_operators=[sqrt, square, sin, exp],
    complexity_of_constants=3,
    complexity_of_operators=[sqrt => 4],
    ...
    maxsize=25  # for gravity force eq
    )
